...
db.restaurants.find({$and:[{cuisine: {$ne:"American"}},{"grades.score": {$gt:70}},{"address.coord.0":{$lt: -65.754168}}]},{_id:0});

// 12. El mateix que l'anterior però sense usar operador $and.
db.restaurants.find({cuisine: {$ne:"American"},"grades.score": {$gt:70},"address.coord.0":{$lt: -65.754168}},{_id:0});
...
db.restaurants.find({$or:[{cuisine:{$regex:/Fish/i}}, {$and:[{cuisine:{$ne:"American"}},{cuisine: {$ne:"Chinese"}}]},{name:{$regex:/^Will/i}}]}, {_id:0, restaurant_id:1, name:1, cuisine:1,});
...
db.restaurants.find({"address.coord":{$near:{$geometry:{type:"Point",coordinates:[-74,40.7]},$maxDistance:5000}}},{_id:0, restaurant_id:1, name:1, "address.street":1, "address.zipcode":1, "address.coord":1});
...
db.restaurants.find({}, {_id:0}).sort({cuisine:1, name:-1});

// 28. Mostrar direccions que no contenen el carrer.
db.restaurants.find({"address.street":{$exists: false}},{_id:0, restaurant_id:1, name:1, address:1});

// 29. Seleccionar documents on el valor de `coordinate` és de tipus Double. Mostrar el name, restaurant_id i coordinades.
db.restaurants.find({"address.coord": {$type:"double"}}, {_id:0, restaurant_id:1, name:1, "address.coord":1});
...
db.restaurants.find({name:{ $regex: /mon/i}},{_id:0, name:1, borough:1, "address.coord.0":1, "address.coord.1":1,cuisine:1});
